...
def visualize_specific_labels(csv_path, labels_to_show=['D', 'I', 'Y'], output_image='ml/compare_diy.png'):
    ...
    # Filter for labels that actually exist in the CSV
    available_labels = [l for l in labels_to_show if l in df['label'].unique()]
    if not available_labels:
        print(f"None of the requested labels {labels_to_show} found in {csv_path}")
    ...
    for label in available_labels:
        ...
        samples = label_df.sample(min(2, len(label_df)))
        
        for index, row in samples.iterrows():
            ax = fig.add_subplot(len(available_labels), 2, plot_idx, projection='3d')
            plot_idx += 1
    ...
    print(f"D-I-Y Comparison saved to {output_image}!")
